...
function getAverageItemPrice(cart){
    let prices = 0;
    for (let i=0;i<cart.items.length;i++){
        prices += cart.items[i].price;
    }
    return prices/(cart.items.length);
}
...
function getAverageSpentPerItem(cart){
    ...
    let spentPerItem = 0;
    for(let i =0;i<cart.items.length;i++){
        spentPerItem += cart.items[i].quantity*cart.items[i].price;
    }
    //total items
    let numberOfItems=0;
    for (let i=0;i<cart.items.length;i++){
        numberOfItems += cart.items[i].quantity;
    }
    return spentPerItem/numberOfItems;
}
...
function mostSpentOnItem(cart) {
    ...
    for (let i =0;i<cart.items.length;i++){
        if (expensiveItem.price*expensiveItem.quantity<cart.items[i].price*cart.items[i].quantity){
            expensiveItem=cart.items[i];
        }
    }
