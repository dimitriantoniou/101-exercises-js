...
function startsAndEndsWithVowel(str){
    if((str[0].match(/[aeiou]/gi))&&(str[str.length-1].match(/[aeiou]/gi))){
        return true;
    }else{
        return false;
    }
...
function onlyNegativeNumbers(arr){
    ...
    for(let i =0;i<arr.length;i++){
        if(arr[i]<0){
            onlyNegatives.push(arr[i]);
        }
    }
    return onlyNegatives;
}
...
function onlyNegativeEvens(arr){
    let onlyNegativeEvens=[];
    for(let i=0;i<arr.length;i++){
        if(arr[i]<0 && arr[i]%2===0){
            onlyNegativeEvens.push(arr[i]);
        }
    }
    return onlyNegativeEvens;
}
...
function onlyNegativeOdds(arr){
    let onlyNegativeOdds=[];
    for(let i =0;i<arr.length;i++){
        if(arr[i]<0 && arr[i]%2!==0){
            onlyNegativeOdds.push(arr[i]);
        }
    }
    return onlyNegativeOdds;
}
...
return unique;
//research more about .from and Set
}
...
function getAverageItemPrice(shoppingCart){
    let numOfTypes = shoppingCart.items.length;
    let sumOfPrices = 0;
    for (let i =0;i<shoppingCart.items.length;i++){
        sumOfPrices += shoppingCart.items[i].price;
    }
    return sumOfPrices/numOfTypes;
}
...
function getAverageSpentPerItem(shoppingCart){
    // sum of array of items[i].quantity * items[i.*price
    let totalPrice =0
    let totalQuant=0;
    for (let i =0;i<shoppingCart.items.length;i++){
        totalPrice += shoppingCart.items[i].quantity*shoppingCart.items[i].price;
    }
    for(let i=0;i<shoppingCart.items.length;i++){
        totalQuant += shoppingCart.items[i].quantity;
    }
...
function mostSpentOnItem(shoppingCart){
    ...
    for (let i=0;i<shoppingCart.items.length;i++){
        if ((shoppingCart.items[i].price*shoppingCart.items[i].quantity)>(mostSpentItem.price*mostSpentItem.quantity)){
            mostSpentItem = shoppingCart.items[i];
        }
    }
